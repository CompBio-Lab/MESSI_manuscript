...
# --- Data ---
# Filter by the covid multiomics only
dname <- "covid_multiomics"
...
out_plot <- df %>%
  ...
  ggtitle("Performance Evaluation of sc Covid Multiomics data only") +
  labs(
    x="Method", y="Mean AUC (5-fold CV)"
  )


output_png_path <- "results/sc/fig6c1_sc_covid_multiomics_auc_performance_bar_plot.png"
