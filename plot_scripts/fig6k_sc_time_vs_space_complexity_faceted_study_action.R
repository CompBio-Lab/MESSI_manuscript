...
# Read both trace in and combined
tr_covid_df <- readr::read_tsv("data/raw/sc_data/covid_data/execution_trace.txt")
tr_htx_df <- readr::read_tsv("data/raw/sc_data/htx_data/execution_trace.txt")
# Read the trace in
trace_df <- bind_rows(
  tr_covid_df, tr_htx_df
  ) %>%
  # Get relevant cols
...
meta_covid_df  <- data.table::fread("data/raw/sc_data/covid_data/parsed_metadata.csv")
meta_htx_df <- data.table::fread("data/raw/sc_data/htx_data/parsed_metadata.csv")
# Merge both and start
metadata_df <- bind_rows(
  meta_covid_df, meta_htx_df
) %>%
...
out_plot <- combined_df %>%
  mutate(method = forcats::fct_reorder(method, realtime_sec),
         study = dataset_name %>% str_replace("_", "-") %>% tools::toTitleCase()) %>%
  mutate(study = case_when(
             stringr::str_detect(study, "Htx") ~ "Htx",
             TRUE ~ study
           )) %>%
ggplot(aes(x=realtime_sec, y=peak_rss_mb, color=method,
  ...
  facet_grid(
    ...
    labeller = labeller(
      ...
      study = label_value,   # keep study as-is
    ncol=2
  )) +
...
output_png_path <- "results/sc/fig6k_sc_time_vs_space_complexity_faceted_study_action.png"
