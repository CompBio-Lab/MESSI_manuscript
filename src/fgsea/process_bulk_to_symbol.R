...
library(stringr)
library(tidyr)
source("process_to_fgsea_scripts/parse_db_utils.R")
...
main <- function(input_path=NULL, output_path=NULL) {
  if (is.null(input_path)) {
    input_path <- "data/raw/bulk_data/all_feature_selection_results.csv"
  }
  ...
  if (is.null(output_path)) {
    output_path <- "data/processed/bulk/feat_selection_symbols.csv"
  }
  ...
  merged <- feat_df %>%
    ...
    # Need to use left join (since the parse list df was operated on subsets of feat)
    # While original feat_df have more feats that already have symbols
    ...
    mutate(symbol = case_when(
      is.na(symbol) ~ feat,
      is.na(symbol) & str_detect(tolower(dataset), "tcga") &
