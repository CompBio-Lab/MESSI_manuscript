...
covid_organ_gsea_input_list <- data.table::fread("data/raw/sc_data/covid_data//all_feature_selection_results.csv") %>%
  filter(dataset_name == "covid_organ") %>%
  ...
  # TODO: dangerous here, rename these RNA is actually from airway as it
  # has epithelial cells and ciliated, vs Protein here that has platalets
  # These organ names should be fixed before
  mutate(organ = ifelse(view == "RNA", "Airway", "Blood")) %>%
  dplyr::select(-view) %>%
  #Now for the covid one need to explode more info out from the feature
  #mutate(stat = abs(coef)) %>%
  arrange(desc(coef)) %>%
  mutate(group = paste(method, dataset, organ, celltype, sep = " | ")) %>%
  group_by(group) %>%
  summarise(stat_vec = list(setNames(coef, feat)), .groups = "drop") %>%
  tibble::deframe()

saveRDS(covid_organ_gsea_input_list, "data/processed/sc/sc_covid_organ_fgsea_list_input.rds")
