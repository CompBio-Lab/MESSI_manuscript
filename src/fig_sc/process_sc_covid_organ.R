# ==============================================================================
# process_sc_covid_organ.R
# Create the plot data for the COVID ORGAN dataset.
# ==============================================================================
source(here::here("src/fig_sc/sc_figure_utils.R"))

process_sc_covid_organ <- function(metric_input_path=NULL, fgsea_input_path=NULL,
  ...
  if (is.null(fgsea_input_path)) {
    fgsea_input_path <- "data/processed/sc/sc_covid_organ_msigdbr_fgsea.csv"
  }
  ...
  if (is.null(output_rds_path)) {
    output_rds_path <- "data/processed/sc/sc_covid_organ_plot_data_list.rds"
  }
  message("\n[process_sc_covid_organ] Start to prepare plot data for figure sc covid organ panel")
  # -----------------------------
  # Configurations
  # Set DATASET to covid_organ to filter
  DATASET         <- "covid_organ"
  ...
  sc_fgsea_data <- load_sc_fgsea_data(
    fgsea_input_path ,
    # Extra grouping columns here compared to COVID multiomics
    group_columns  = c("method", "dataset", "organ", "celltype"),
    dataset_filter = DATASET,
    padj_group_vars = c("method", "dataset", "organ", "celltype")
  )
  # Now two things to get
  # Count of significant pathways in each method, note here facet by organ NOT view
  sig_pathways_count <- count_sc_sig_pathways(sc_fgsea_data, facet_var = "organ")
  ...
  # And save it to file
  message("[process_sc_covid_organ] Saving plot data to file: ", output_rds_path)
  saveRDS(output_plot_data_list, output_rds_path)
}

# Execute it
process_sc_covid_organ(
  metric_input_path="data/raw/sc_data/covid_data/metrics.csv",
  fgsea_input_path="data/processed/sc/sc_covid_organ_msigdbr_fgsea.csv",
  trace_path="data/raw/sc_data/covid_data/execution_trace.txt",
  metadata_path="data/raw/sc_data/covid_data/parsed_metadata.csv",
