...
feat_sel_real <- here(fig_path, "feature_selection_real.rds") |> readRDS() |>
  ggplotify::as.ggplot()
perf_real <- here(fig_path, "perf_evaluation_real.rds") |> readRDS() |>
  ggplotify::as.ggplot()
# Figs on sim dataset
...
plot_real_grid_top_row <- plot_grid(
  perf_real +
    ggtitle("Mean AUC (5-fold CV) ranking in real datasets") +
    theme(plot.title = element_text(hjust=0.5, size=10)),
  feat_sel_real +
    ggtitle("Spearson rank correlation of feature selection results on real datasets") +
    theme(plot.title = element_text(hjust=0.5, size=10)),
  labels = c("A", "B"),
  hjust = -1,
  nrow = 1
