...
df_covid_counts <- msigdbr_df %>%
  mutate(method = standardize_method_names(method)) %>%
  mutate(is_covid_pathway = str_detect(
    pathway,
    regex(
      "Viral|Interferon|Cytokine|Immune|SARS|Coronavirus|Replication|Transcription",
      ignore_case = TRUE
    )
  )) %>%
  filter(is_covid_pathway) %>%
  group_by(method, celltype, organ) %>%
  summarise(n_sig = sum(padj < 0.2), .groups = "drop")

out_plot <- ggplot(df_covid_counts,
       aes(y = reorder(method, n_sig, FUN=sum), x = n_sig, fill = celltype)) +
...
output_png_path <- "results/sc/fig6m_sc_covid_organ_pathways.png"
