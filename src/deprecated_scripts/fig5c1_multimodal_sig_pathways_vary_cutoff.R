...
plot_fig5c1_multimodal_n_sig_pathway_vary_cutoff <- function(input_path=NULL,
                                                             output_path=NULL,
                                                             text_size=7) {
  if (is.null(input_path)) {
    input_path <- "data/processed/multimodal/multimodal_msigdbr_fgsea.csv"
  }

  if (is.null(output_path)) {
    output_path <-  "results/multimodal/fig5c1_multimodal_sig_pathways_vary_cutoff.png"
  }
  ...
  sig_counts <- expand_grid(
    ...
    method = unique(multimodal_msigdbr_df$method)
  ) %>%
    rowwise() %>%
    mutate(
      n_sig = sum(
        multimodal_msigdbr_df$padj < threshold &
          multimodal_msigdbr_df$method == method,
        na.rm = TRUE
      )
    ) %>%
    ungroup()
...
plot_fig5c1_multimodal_n_sig_pathway_vary_cutoff(
  input_path="data/processed/multimodal/multimodal_msigdbr_fgsea.csv",
  output_path="results/multimodal/fig5c1_multimodal_sig_pathways_vary_cutoff.png",
  text_size=12
)
