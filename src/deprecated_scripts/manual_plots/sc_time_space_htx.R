...
combined_df <- left_join(
  ...
  # =======================
# Filter for covid multiomics data only
filter(str_detect(tolower(dataset_name), "htx")) %>%
...
out_plot <- combined_df %>%
  ...
  mutate(m0ethod = forcats::fct_reorder(method, realtime_sec)) %>%
  ggplot(aes(x=realtime_sec, y=peak_rss_mb, color=color_label,
             label=method)) +
  geom_point(size=8) +
  scale_y_log10(labels = scales::label_comma(),
                breaks = c(300, 500, 700, 1000, 1500)) +
...
ggsave("aaa.svg", the_plot, width=9, height=15, dpi=1200)
