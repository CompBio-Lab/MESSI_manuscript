...
\urlstyle{same}
\hypersetup{
  pdftitle={MESSI: Multimodal Experiments with SyStematic Interrogation using nextflow},
...
\title{MESSI: Multimodal Experiments with SyStematic Interrogation using nextflow}
...
\hypertarget{computational-resources-usage}{%
\subsubsection{Computational Resources Usage}\label{computational-resources-usage}}
...
Now taking into account of computational cost of the methods \protect\hyperlink{computational-resources-usage}{Computational Resources Usage}, we then proceed to examine the real world datasets as describe in table \ref{tab:benchmark-data-table} just like we do with simulation studies. The scripts to clean these data could be found under {[}insert link here\ldots{]}.
...
The heatmap (Fig \ref{fig:fgsea-analysis-plot} panel B left) displays the proportion of significant cells (i.e., features or pathways) identified at a relaxed significance threshold (\(p < 0.2\)) for each combination of disease (rows) and tissue (columns). Clustering was applied to both dimensions, which helps reveal patterns of tissue-disease associations. For example, \textbf{Thyroid Cancer} shows a strong, specific association with the \textbf{thyroid tissue}, with a proportion of 1.00 across multiple methods. Similarly, \textbf{Bile Duct Cancer} and \textbf{Kidney cancers} (Chromophobe and Renal Clear Cell) show localized associations to \textbf{liver} and kidney tissues respectively, suggesting the methods are capturing biologically meaningful tissue relevance. Interestingly, \textbf{Autism} displays moderate signals in brain tissues, while \textbf{Alzheimer's} and \textbf{Skin Cancer} show relatively sparse or distributed patterns, indicating possible heterogeneity or weaker tissue-specific signals in those conditions.
