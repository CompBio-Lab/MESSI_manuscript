...
```{r confusion-mat-tab, results="asis"}
conf_matrix <- data.frame(
  positive = c("True Positive (TP)", "False Positive (FP)"),
  negative = c("False Negative (FN)", "True Negative (TN)")
)

rownames(conf_matrix) <- c("Predicted Positive", "Predicted Negative")

# Display using kable
knitr::kable(conf_matrix, format = "latex", booktabs = T, 
             caption = "Confusion matrix of binary classification", 
             col.names = c("Actual Positive", "Actual Negative")) %>%
