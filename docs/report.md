...
\caption{Mean runtime and memory usage of methods across computational stages.(A) Duration (in seconds) of each method separated by stage: preprocessing, model training, prediction, and feature selection. (B) Corresponding RAM memory usage (in megabytes) for the same stages. Bars represent mean values with error bars indicating standard deviations. Each stage is color-coded: preprocessing (yellow), training (blue), prediction (green), and feature selection (pink).}(\#fig:computational-resource-plot)
\end{figure}
...
Overall, these analyses highlight both the tissue specificity of disease signals and the relative contribution of various integrative analysis methods, reinforcing the importance of method selection in multi-omics studies. The results also underscore the biological plausibility of several associations and emphasize the strength of methods like MOFA and DIABLO in uncovering such patterns.
